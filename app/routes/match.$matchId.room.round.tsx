...
import type { DiscardCard } from "@prisma/client";
...
export async function loader({ request, params }: LoaderArgs) {
  ...
  if (!match || !match.Seats) throw new Error("No match was found @ room load");
  const seats = match.Seats;
  const userSeatIdx = seats.findIndex((seat) => seat.occupantId === user.id);
  ...
  const playerData = {
    ...match,
    seats: relSeating,
    round: match.Rounds[match.Rounds.length - 1]
  };
  return json({ game: playerData });
}

export default function RoundIndexRoute() {
...
function Table({ data }: any) {
  console.log(data);
  const { seats, round } = data;
  const { deckCount } = round;
    ...
    <div className="absolute inset-x-[11%] bottom-[20.5%] top-[18.5%] flex-col">
      <div className="relative grid h-full grid-cols-12 grid-rows-8">
        <div className="row-span-1 col-span-11">
          {top && (
            <div className="justify-left flex h-8 w-full items-center justify-between rounded-full bg-primary-green-8">
              ...
              <p className="mx-1.5 flex transform truncate text-3xl font-extrabold text-white ">
                {top.totalScore}
              </p>
            </div>
          )}
        ...
        <div className="row-start-9 col-span-11 col-start-2 flex flex-col justify-end">
          <div className="flex h-8 w-full items-center justify-between rounded-full bg-primary-green-8">
            <p className="mx-1.5 flex transform truncate text-3xl font-extrabold text-white ">
              {bot.totalScore}
            </p>
        ...
        <div className="absolute inset-x-0 top-[103.5%]">
          <HeroGUI Hand={<HeroHand cards={testInitHand} />} />
        </div>
